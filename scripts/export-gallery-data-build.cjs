// Standalone gallery data export script for Vercel builds
// This script runs during build time and doesn't depend on TypeScript compilation

const fs = require('fs');
const path = require('path');
const matter = require('gray-matter');

async function loadGalleryPostsFromMarkdown() {
  const posts = [];
  ...
  try {
    ...
    for (const modelDirName of modelDirs) {
      ...
      // Check for new structure: gallery subdirectories with index.md
      ...
      for (const gallerySlug of galleryDirs) {
        ...
        if (fs.existsSync(indexFile)) {
          const post = loadPostFromFile(indexFile, `${modelSlug}/${gallerySlug}`, modelSlug);
      ...
      for (const fileName of markdownFiles) {
        ...
        const post = loadPostFromFile(filePath, `${modelSlug}/${gallerySlug}`, modelSlug);
        if (post) posts.push(post);
      }
    }
...
function loadPostFromFile(filePath, slug, modelSlug) {
  try {
...
async function exportGalleryData() {
  try {
    ...
    const outputPath = path.join(apiDir, 'gallery-data.json');
